...
def down_song(song, singer, filetype):
    ...
    conn.close()
"""
def save_singer(name,country,year,created):

    singerlist = sbs.show_singer();
    print("singerlist:", singerlist)

    if name.lower() not in singerlist:
        #sql연결
        conn = sql.connect(host='', user='', password='', charset='utf8', db='')
        ...
        sql_query = "insert into singer(name,country,year,created) values(%s,%s,%s,NOW())"
        #cursor 객체의 execute 메서드를 이용해서 sql  문장을 실행할 수 있음.
        cursor.execute(sql_query,(name,country,year))

        # 실행한 sql 결과를 서버에 확정 반영하기
        conn.commit()

        print("test_:", name,country,year)

        conn.close()

def save_song(song,filetype,genre,feat,lyrics,singer,year,created):
    ...
    sql_query = "insert into song(song,filetype,singer,created) values(%s,%s,%s,%s,%s,%s,%s,NOW())"
    #cursor 객체의 execute 메서드를 이용해서 sql  문장을 실행할 수 있음.
    cursor.execute(sql_query,(song,filetype,singer))
